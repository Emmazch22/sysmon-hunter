...
class TestProcessTree:
    def test_ancestry_is_root_first(self, tree, at) -> None:
        ...
        tree.observe(
            make_event(
                guid="c", parent_guid="w", image=r"C:\W\cmd.exe", timestamp=at(1)
            )
        )
        tree.observe(
            make_event(
                guid="p", parent_guid="c", image=r"C:\W\powershell.exe", timestamp=at(2)
            )
        )
    ...
    def test_root_is_topmost_known_ancestor(self, tree, at) -> None:
        ...
        tree.observe(
            make_event(guid="c", parent_guid="w", image="cmd.exe", timestamp=at(1))
        )
    ...
    def test_ancestry_stops_at_the_edge_of_knowledge(self, tree, at) -> None:
        ...
        tree.observe(
            make_event(
                guid="c", parent_guid="missing", image="cmd.exe", timestamp=at(0)
            )
        )
    ...
    def test_later_events_backfill_missing_detail(self, tree, at) -> None:
        ...
        tree.observe(
            make_event(guid="p", image=r"C:\W\powershell.exe", timestamp=at(0))
        )
...
class TestIncidentCorrelation:
    def test_detections_on_one_tree_form_one_incident(
        self, incidents, tree, at
    ) -> None:
        tree.observe(make_event(guid="w", image="WINWORD.EXE", timestamp=at(0)))
        tree.observe(
            make_event(guid="c", parent_guid="w", image="cmd.exe", timestamp=at(1))
        ...
        tree.observe(
            make_event(
                guid="p", parent_guid="c", image="powershell.exe", timestamp=at(2)
            )
        )

        i1 = incidents.correlate(
            detect(make_event(guid="c", parent_guid="w"), "SYS-001", when=at(1))
        )
        i2 = incidents.correlate(
            detect(make_event(guid="p", parent_guid="c"), "SYS-002", when=at(2))
        )

        assert i1.id == i2.id
        assert len(i2.detections) == 2

    def test_incident_chain_reflects_the_deepest_detection(
        self, incidents, tree, at
    ) -> None:
    ...
    def test_quiet_period_opens_a_fresh_incident(self, incidents, tree, at) -> None:
        ...
        second = incidents.correlate(
            detect(make_event(guid="p"), "SYS-001", when=at(660))
        )
        assert first.id != second.id


class TestActionability:
    def test_critical_detection_is_immediately_actionable(
        self, incidents, tree, at
    ) -> None:
        ...
        incident = incidents.correlate(
            detect(
                make_event(guid="p"), "SYS-010", severity=Severity.CRITICAL, when=at(0)
            )
        )
        assert incidents.is_actionable(incident)

    def test_a_single_medium_detection_is_not_actionable(
        self, incidents, tree, at
    ) -> None:
        ...
        incident = incidents.correlate(
            detect(
                make_event(guid="p"), "SYS-050", severity=Severity.MEDIUM, when=at(0)
            )
        )
    ...
    def test_three_detections_are_actionable_by_volume(
        self, incidents, tree, at
    ) -> None:
...
class TestIncidentScoring:
    def test_severity_derives_from_cumulative_score(self, incidents, tree, at) -> None:
        ...
        tree.observe(make_event(guid="p", image="powershell.exe", timestamp=at(0)))
        incidents.correlate(
            detect(make_event(guid="p"), "A", severity=Severity.HIGH, when=at(0))
        )
        incident = incidents.correlate(
            detect(make_event(guid="p"), "B", severity=Severity.HIGH, when=at(1))
        )
    ...
    def test_techniques_are_deduplicated_and_sorted(self, incidents, tree, at) -> None:
        tree.observe(make_event(guid="p", image="powershell.exe", timestamp=at(0)))
        incidents.correlate(
            detect(make_event(guid="p"), "A", attack=["T1059", "T1027"], when=at(0))
        )
        incident = incidents.correlate(
            detect(make_event(guid="p"), "B", attack=["T1059", "T1003"], when=at(1))
        )
        assert incident.techniques == ["T1003", "T1027", "T1059"]


class TestSubtree:
    """The full branching tree an incident spans, for the process-tree view."""

    def test_subtree_includes_all_branches(self, tree, at) -> None:
        """A foothold that spawned several children must show all of them, not
        just the ancestry of one detection."""
        tree.observe(make_event(guid="root", image="explorer.exe", timestamp=at(0)))
        tree.observe(
            make_event(guid="a", parent_guid="root", image="cmd.exe", timestamp=at(1))
        )
        tree.observe(
            make_event(
                guid="b", parent_guid="root", image="powershell.exe", timestamp=at(1)
            )
        )
        tree.observe(
            make_event(guid="c", parent_guid="a", image="whoami.exe", timestamp=at(2))
        )

        nodes = tree.subtree("LAB-WIN11", "root")
        names = {n["name"] for n in nodes}
        assert names == {"explorer.exe", "cmd.exe", "powershell.exe", "whoami.exe"}

    def test_subtree_carries_parent_links(self, tree, at) -> None:
        """Each node reports its parent, so the caller can rebuild the hierarchy."""
        tree.observe(make_event(guid="root", image="explorer.exe", timestamp=at(0)))
        tree.observe(
            make_event(
                guid="child", parent_guid="root", image="cmd.exe", timestamp=at(1)
            )
        )

        nodes = tree.subtree("LAB-WIN11", "root")
        child = next(n for n in nodes if n["name"] == "cmd.exe")
        assert child["parent_guid"] == "root"

    def test_subtree_is_scoped_per_host(self, tree, at) -> None:
        """A subtree must not pull in a same-GUID process from another host."""
        tree.observe(make_event(host="A", guid="root", image="a.exe", timestamp=at(0)))
        tree.observe(
            make_event(
                host="A",
                guid="child",
        ...
        nodes = tree.subtree("A", "root")
        names = {n["name"] for n in nodes}
        assert "b-child.exe" not in names

    def test_subtree_of_unknown_root_is_empty(self, tree) -> None:
        assert tree.subtree("LAB-WIN11", "ghost") == []
