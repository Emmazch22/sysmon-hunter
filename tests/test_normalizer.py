...
class TestPayloadShapes:
    def test_winlogbeat_envelope(self) -> None:
        event = normalize({
            "winlog": {"event_id": 1, "computer_name": "WS01",
    ...
    def test_flat_payload(self) -> None:
        ...
        event = normalize({"EventID": 1, "Image": r"C:\W\cmd.exe",
                           "ParentImage": r"C:\O\WINWORD.EXE"})
        assert event.event_id == 1
        assert event.parent_image == r"C:\O\WINWORD.EXE"

    def test_host_from_ecs_style_object(self) -> None:
        event = normalize({"winlog": {"event_id": 1, "event_data": {}},
                          "host": {"name": "LAPTOP-7"}})
        assert event.host == "LAPTOP-7"

    def test_missing_host_defaults_rather_than_crashing(self) -> None:
        assert normalize({"winlog": {"event_id": 3, "event_data": {}}}).host == "unknown"
...
class TestFieldHandling:
    def test_unmapped_fields_are_reachable_via_raw(self) -> None:
        ...
        event = normalize({"winlog": {"event_id": 10, "event_data": {
            "TargetImage": r"C:\W\lsass.exe", "GrantedAccess": "0x1410"}}})
